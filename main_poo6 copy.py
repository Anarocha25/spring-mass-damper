...
class MainWindow(QtWidgets.QDialog):
    ...
    def run(self):
        ...
        if len(self.param.warnings) != 0:
            ...
            self.add_sys_ui2()
            if self.param.save:
                self.worker_save = SavePlot(parent=self)
                self.worker_save.start()
                self.worker_save.complete_worker.connect(lambda: self.worker_save.terminate())
                self.worker_save.complete_worker.connect(lambda: self.worker_save.deleteLater())
                self.worker_save.complete_worker.connect(lambda: self.worker_save.video.close())
...
class SavePlot(QtCore.QThread):
    ...
    def __init__(self, parent):    
        ...
        self.parent = parent
        self.video = vd.io.Video("system.mp4", fps=self.parent.param.fram_res_par, backend='opencv') # backend='opencv'
        #self.video.action(elevation_range=80, azimuth_range=90)
        self.video.action(cam1={"azimuth":90, "elevation":80, "roll":90})
    ...
    def run(self):
        ...
        # Create mass spring damper system
        sys1 = MassSpringDamperSys1(self.parent.param.mass_par, self.parent.param.damp_cons_par, self.parent.param.spring_cons_par, \
                                  self.parent.param.spring_leng_par, self.parent.param.side_cube_par, self.parent.param.end_time_par, \
                                  self.parent.param.fram_res_par, self.parent.param.amp_par, self.parent.param.freq_par)
        ...
        if self.parent.param.two_sys:
            sys2 = MassSpringDamperSys2(self.sys1.yc, self.parent.param.mass_par2, self.parent.param.damp_cons_par2, self.parent.param.spring_cons_par2, \
                                    self.parent.param.spring_leng_par2, self.parent.param.side_cube_par2, self.parent.param.end_time_par, \
                                    self.parent.param.fram_res_par, self.parent.param.amp_par2, self.parent.param.freq_par2)
