...
def main(mass, damp_cons, spring_cons, spring_leng, side_cube, end_time, fram_res, amp, freq, two_sys=False):
    ...
    if two_sys:
        ...
        line_x2 = line([x_vals[0], block_ratio, np.median(yc)+init_point+0.8+block_ratio], [x_vals[-1]+2*dt, block_ratio, np.median(yc)+init_point+0.8+block_ratio])
        
        txt_time2 = text('time', 0.05*side_cube, (x_vals[-1]+2*dt, block_ratio, np.median(yc)+init_point+0.8+block_ratio), 270)
