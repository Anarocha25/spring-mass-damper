...
# class UpdateSystem(QtCore.QThread):
...
#     def run(self):
#         for i, yc_i in enumerate(self.parent.sys1.yc):
#             print(i, yc_i)
#             self.parent.vp.actors[6].pos(self.parent.sys1.pos_points[i])
#             self.parent.vp.actors[2].pos([0, 0, yc_i + self.parent.sys1.block_ratio])  
#             self.parent.vp.actors[-1].pos(0, self.parent.sys1.block_ratio, yc_i + self.parent.sys1.block_ratio)
#             self.parent.vp.actors[3].stretch([self.parent.sys1.side_cube/4, 0,-0.8],[self.parent.sys1.side_cube/4, 0, yc_i])
#             self.parent.vp.actors[4].stretch([-self.parent.sys1.side_cube/4, 0, -0.2], [-self.parent.sys1.side_cube/4, 0, yc_i])
#             self.parent.vp.interactor.Render()

            # self.listener.emit()

class Runnable(QtCore.QRunnable):
    emitter = QtCore.pyqtSignal(QtCore.QObject)
    ...
    def run(self):
        vp = self.parent.vp
        for i, yc_i in enumerate(self.parent.sys1.yc):
            vp.actors[6].pos(self.parent.sys1.pos_points[i])
            vp.actors[2].pos([0, 0, yc_i + self.parent.sys1.block_ratio])  
            vp.actors[-1].pos(0, self.parent.sys1.block_ratio, yc_i + self.parent.sys1.block_ratio)
            vp.actors[3].stretch([self.parent.sys1.side_cube/4, 0,-0.8],[self.parent.sys1.side_cube/4, 0, yc_i])
            vp.actors[4].stretch([-self.parent.sys1.side_cube/4, 0, -0.2], [-self.parent.sys1.side_cube/4, 0, yc_i])
            #vp.interactor.Render()
            self.emitter.emit(vp)
...
class MainWindow(QtWidgets.QDialog):
    ...
    def update_ui2(self):
        ...
        self.sys1 = MassSpringDamper(self.param.mass_par, self.param.damp_cons_par, self.param.spring_cons_par, self.param.spring_leng_par, \
                        self.param.side_cube_par, self.param.end_time_par, self.param.fram_res_par, self.param.amp_par, self.param.freq_par)

        wall = vd.shapes.Box(pos=(0, 0, -0.8), length=1.5 * self.sys1.side_cube, width=1.5 * self.sys1.side_cube, height=0.05 * self.sys1.side_cube, c=(128,128,128))  
        const = 1#2 if self.param.two_sys_par else 1
        height_surf = const*(0.8 + np.max(self.sys1.yc) + self.sys1.side_cube)
        surface = vd.shapes.Box(pos=(0, -self.sys1.side_cube, -0.8 + (height_surf/2)), length=2 * self.sys1.side_cube, width=0.02, height=height_surf, alpha=0)
        
        # Create renderer and add the vedo objects
        self.vp = vd.Plotter(axes=4, qtWidget=self.vtkWidget)

        self.vp += [wall, surface, self.sys1.block, self.sys1.spring, self.sys1.tube, self.sys1.cylinder, self.sys1.redpt, self.sys1.line_cos, self.sys1.line_x, self.sys1.line_y, self.sys1.txt_U, self.sys1.txt_time, self.sys1.redpt_block]
        self.vp.show(azimuth=90, elevation=80, roll=90)

        # Set-up the rest of the Qt window
        new_layout.addWidget(self.vtkWidget)
        #new_layout.addWidget(self.frame)
        #self.frame.setLayout(self.new_layout)
        #self.setCentralWidget(self.frame)
        self.update_tab2_layout(new_layout)

    def update_tab2_layout(self, new_layout):
    ...
    def run(self):
        ...
        if len(self.param.warnings) != 0:
            ...
            runnable.emitter.connect(lambda vp:vp.interactor.Render())
            pool.start(runnable)
